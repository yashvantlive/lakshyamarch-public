...
export default function NeetResultsShowcase({
  neet = [],
  className,
}: {
  neet?: ResultStudent[];
  ...
  const years = useMemo(() => {
    const set = Array.from(new Set(neet.map((d) => d.year).filter(Boolean))) as number[];
  ...
  const visibleByYear = useMemo(() => {
    ...
    years.forEach((y) => {
      const filtered = neet.filter((d) => d.year === y);
          ...
          <Stagger className="grid grid-cols-2 gap-4 sm:gap-6 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {visibleByYear.get(year)?.map((student) => (
              <StaggerItem key={student.id} variants={staggerItem}>
                <ResultCard student={student} category="NEET" />
              </StaggerItem>
            ))}
          </Stagger>
        </div>
